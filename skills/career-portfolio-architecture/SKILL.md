---
name: career-portfolio-architecture
...
若用户要求从资料开始建设网站、知识库、Word和发布包，立即转由 `career-portfolio-builder` 主持全流程；本Skill只负责定位、信息架构与载体分工，并把结论交回主Skill的事实台账和manifest。
...
涉及特定国家或招聘市场时读取[目标市场适配协议](../career-assets/references/market-localization.md)，不要把某一地区的页面、隐私或语言习惯写成全球默认。
...
涉及职责、数据视觉权重或前雇主敏感信息时，分别调用 `evidence-weighted-case-storytelling`、`portfolio-multi-format-qa`、`former-employer-data-redactor`。
